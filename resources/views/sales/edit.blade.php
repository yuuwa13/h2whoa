...
<div class="container">
    ...
    <form action="{{ route('sales.update', $sale->sale_id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="number" class="form-control @error('order_id') is-invalid @enderror" id="order_id" name="order_id" value="{{ $sale->order_id }}" readonly>
            @error('order_id')
        ...
        <div class="mb-3">
            ...
            <select class="form-select @error('sale_type') is-invalid @enderror" id="sale_type" name="sale_type" required>
                <option value="phone" {{ $sale->sale_type === 'phone' ? 'selected' : '' }}>Phone</option>
                <option value="on-site" {{ $sale->sale_type === 'on-site' ? 'selected' : '' }}>On-Site</option>
            </select>
            @error('sale_type')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        ...
        <div class="mb-3">
            ...
            <table class="table table-bordered" id="saleDetailsTable">
                ...
                <tbody>
                    ...
                    <tr>
                        <td><input type="text" name="sale_details[{{ $index }}][product_name]" class="form-control" value="{{ $detail->product_name }}" required></td>
                        <td><input type="number" name="sale_details[{{ $index }}][quantity]" class="form-control" value="{{ $detail->quantity }}" required></td>
                        <td><input type="number" step="0.01" name="sale_details[{{ $index }}][price_per_unit]" class="form-control" value="{{ $detail->price_per_unit }}" required></td>
                        <td><input type="number" step="0.01" name="sale_details[{{ $index }}][total_price]" class="form-control" value="{{ $detail->total_price }}" readonly></td>
